...
{
  // G4cout<<"<<------------RunAction::RunAction(PrimaryGeneratorAction* kin,HistoManager* histo)-------------------->>"<<G4endl;
...
{ 
  // G4cout<<"<<------------RunAction::BeginOfRunAction(const G4Run*)-------------------->>"<<G4endl;
  //initialize arrays
  //
...
{
  // G4cout<<"<<------------RunAction::ParticleCount(G4String name, G4double Ekin)-------------------->>"<<G4endl;
  fParticleCount[name]++;
  fEmean[name] += Ekin;
  //update min max
...
{
  // G4cout<<"<<------------RunAction::Balance(G4double Ekin, G4double Pbal)-------------------->>"<<G4endl;
  fDecayCount++;
  fEkinTot[0] += Ekin;
  //update min max  
...
{
  // G4cout<<"<<------------RunAction::EventTiming(G4double time)-------------------->>"<<G4endl;
  fTimeCount++;  
...
{
  // G4cout<<"<<------------RunAction::PrimaryTiming(G4double ptime)-------------------->>"<<G4endl;
  fPrimaryTime += ptime;
}
...
{
  // G4cout<<"<<------------RunAction::EndOfRunAction(const G4Run* run)-------------------->>"<<G4endl;
 ...
 G4cout << "\n   Activity of " << partName << " = "
  ...
  // //print number of particles totally inside the ScoringVolume

  // G4cout << "\n  In this run, there are " << svParticleCount << " e- tracks (from "
  //        << nevents <<" events) completely inside the scoring volume--Gas.  \n" <<G4endl; 
