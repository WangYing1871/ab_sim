...
:myAction(mpga)   //  ??????????
{
  G4cout<<"<<------------PrimaryGeneratorMessenger::PrimaryGeneratorMessenger(PrimaryGeneratorAction * mpga)-------------------->>"<<G4endl;
  mydetDirectory = new G4UIdirectory("/mydet/");
  mydetDirectory->SetGuidance("NTD_Ge detector control commands.");
...
{
  G4cout<<"<<------------PrimaryGeneratorMessenger::~PrimaryGeneratorMessenger()-------------------->>"<<G4endl;
  delete genCmd;
...
{
  G4cout<<"<<------------PrimaryGeneratorMessenger::SetNewValue(G4UIcommand* command, G4String newValue)-------------------->>"<<G4endl;
  G4cout << "newValue is " << newValue << G4endl;
  if( command==genCmd)
...
{
  G4cout<<"<<------------PrimaryGeneratorMessenger::GetCurrentValue(G4UIcommand * command)-------------------->>"<<G4endl;
  G4String cv;
  if( command == genCmd)
  {
